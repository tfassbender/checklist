...
Check if there's already a server running on port 4715.
...
```bash
curl -s -o /dev/null -w "%{http_code}" http://localhost:4715/api/games
```
...
The skill succeeds when the server responds to HTTP requests on http://localhost:4715.
