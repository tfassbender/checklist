...
description: Stop the Quarkus dev server. Use when needing to stop the backend server, shut down quarkusDev, or free up port 4715.
allowed-tools: Bash(netstat *), Bash(taskkill *), Bash(lsof *), Bash(kill *)
---

# Stop Quarkus Dev Server

Stop the Quarkus dev server for this project. This skill checks for a running server on port 4715 and stops it if found.
...
Check if there's a server running on port 4715.

**On Windows:**
```bash
netstat -ano | findstr :4715 | findstr LISTENING
```

**On Linux/macOS:**
```bash
lsof -i :4715 | grep LISTEN
```
...
- The server was stopped and port 4715 is now free
